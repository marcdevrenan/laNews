...
class Crawler:
    ...
    def extract_news(self):
        ...
        for article in news:
            ...
            description = article.find_element(By.CLASS_NAME, LO.ARTICLE_DESCRIPTION).text
            articles.append(Article(
                title = title,
                date = article.find_element(By.CLASS_NAME, LO.ARTICLE_PUBLISH_DATE).text,
                description = description,
                picture = Crawler.download_picture(article, title),
                search_phrase_count = Toolbox.search_phrase_count(SEARCH_PHRASE, title, description),
    ...
    def download_picture(article, title):
        try:
            ...
            picture = article.find_element(By.CLASS_NAME, LO.ARTICLE_PICTURE).get_attribute('srcset')
